...
import {
    KLIENTO_SERVICE_OID,
    MAX_TOKEN_BUNDLE_OCTETS,
} from './serviceConfig.js';
import {
    AUDIENCE,
    CLAIM_KEY,
    CLAIM_VALUE,
} from './testUtils/klientoStubs.js';
import { ORG_NAME, USER_NAME } from './testUtils/veraidStubs.js';
import { Token } from './Token.js';
import { TokenBundle } from './TokenBundle.js';

const MOCK_TRUST_CHAIN = await MockTrustChain.generate(ORG_NAME, USER_NAME, addMinutes(new Date(), 10));
...
describe('TokenBundle', () => {
    describe('sign', () => {
        it('should sign token with specified chain and key', async () => {
            const tokenBundle = await TokenBundle.sign(TOKEN, MOCK_TRUST_CHAIN.signerPrivateKey, MOCK_TRUST_CHAIN.chain, addSeconds(new Date(), 10));
            
            const { member } = await tokenBundle.signatureBundle.verify(undefined, KLIENTO_SERVICE_OID, new Date(), MOCK_TRUST_CHAIN.dnssecTrustAnchors);
        ...
        it('should encapsulate specified token', async () => {
            ...
            const { plaintext } = await tokenBundle.signatureBundle.verify(undefined, KLIENTO_SERVICE_OID, new Date(), MOCK_TRUST_CHAIN.dnssecTrustAnchors);
            
            const tokenSerialised = Buffer.from(TOKEN.serialise());
            
            expect(Buffer
                .from(plaintext)
                .equals(tokenSerialised)).toBeTruthy();
        });
        
        it('should set specified expiry date', async () => {
            const expiry = setMilliseconds(new Date(), 0);
            
            const tokenBundle = await TokenBundle.sign(TOKEN, MOCK_TRUST_CHAIN.signerPrivateKey, MOCK_TRUST_CHAIN.chain, expiry, {
                startDate: expiry,
            });
            
            await expect(tokenBundle.signatureBundle.verify(
                undefined,
                KLIENTO_SERVICE_OID,
                expiry,
        ...
        it('should set start date to current date by default', async () => {
            ...
            const tokenBundle = await TokenBundle.sign(TOKEN, MOCK_TRUST_CHAIN.signerPrivateKey, MOCK_TRUST_CHAIN.chain, addSeconds(startDate, 10));
    ...
    describe('verify', () => {
        it('should use current date by default', async () => {
            const tokenBundle = await TokenBundle.sign(TOKEN, MOCK_TRUST_CHAIN.signerPrivateKey, MOCK_TRUST_CHAIN.chain, addSeconds(new Date(), 5), // Use a very narrow window
);
            
            await expect(tokenBundle.verify(AUDIENCE, {
        ...
        it('should refuse malformed tokens', async () => {
            ...
            const signatureBundle = await SignatureBundle.sign(malformedToken, KLIENTO_SERVICE_OID, MOCK_TRUST_CHAIN.chain, MOCK_TRUST_CHAIN.signerPrivateKey, addSeconds(new Date(), 10), {
                shouldEncapsulatePlaintext: true,
            });
            
            const invalidTokenBundle = TokenBundle.deserialise(signatureBundle.serialise());
            
            await expect(invalidTokenBundle.verify(AUDIENCE, {
                trustAnchors: MOCK_TRUST_CHAIN.dnssecTrustAnchors,
        ...
        it('should refuse token with mismatching audience', async () => {
            ...
            await expect(tokenBundle.verify(
                differentAudience, {
                    trustAnchors: MOCK_TRUST_CHAIN.dnssecTrustAnchors,
                },
    ...
    describe('serialise', () => {
        it('should serialise token bundle', async () => {
            ...
            expect(Buffer
                .from(tokenBundleSerialised)
                .equals(Buffer.from(signatureBundleSerialised))).toBeTruthy();
        });
    });
    ...
    describe('deserialise', () => {
        it('should refuse serialisations greater than the maximum allowed', () => {
            ...
            expect(() => TokenBundle.deserialise(bundle)).toThrowError(new Error('Token bundle serialisation is too large: ' + `${bundle.byteLength} bytes (max ${MAX_TOKEN_BUNDLE_OCTETS} octets)`));
        });
        ...
        it('should process serialisations of exactly the maximum allowed', () => {
            ...
            expect(() => TokenBundle.deserialise(bundle)).toThrowError(new Error('Token serialisation is malformed'));
        });
        ...
        it('should return well-formed token bundle', async () => {
            ...
            expect(Buffer
                .from(bundle)
                .equals(Buffer.from(deserialisedTokenBundle.serialise()))).toBeTruthy();
        });
    });
});
